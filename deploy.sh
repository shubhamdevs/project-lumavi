...
gcloud run deploy lumavi-backend \
...
BACKEND_URL=$(gcloud run services describe lumavi-backend \
  --platform=managed \
  --region="${REGION}" \
  --project="${PROJECT_ID}" \
...
gcloud run services update lumavi-backend \
  --platform=managed \
  --region="${REGION}" \
  --project="${PROJECT_ID}" \
...
gcloud run deploy lumavi-frontend \
...
FRONTEND_URL=$(gcloud run services describe lumavi-frontend \
  --platform=managed \
  --region="${REGION}" \
  --project="${PROJECT_ID}" \
  --format="value(status.url)")
