...
import { saveBrandSection } from '@/app/actions/brand';
...
export default function BrandHub({ initialData, workspaceId }: BrandHubProps) {
  ...
  const handleSaveVisual = async () => {
    ...
    const result = await saveBrandSection(workspaceId, {
    ...
    if (result.success) {
      ...
      toast.error(result.error || 'Failed to save visual identity');
    }
  };
  ...
  const handleSaveAudience = async () => {
    ...
    if (result.success) {
      ...
      toast.error(result.error || 'Failed to save audience intelligence');
    }
  };
  ...
  const handleSaveTypography = async () => {
    ...
    if (result.success) {
      ...
      toast.error(result.error || 'Failed to save typography guidelines');
    }
  };
