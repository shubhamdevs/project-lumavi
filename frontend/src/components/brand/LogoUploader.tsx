...
export default function LogoUploader({
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const response = await uploadBrandLogo(token, workspaceId, base64Data, file.type, variant);
      ...
      if (variant === 'light') {
        ...
        try {
          const analyzeData = await analyzeLogo(token, base64Data, file.type);
          setIsAnalyzing(false);
          if (analyzeData && Object.keys(analyzeData).length > 0) {
            onAnalysisComplete(analyzeData as Partial<BrandSignals>);
